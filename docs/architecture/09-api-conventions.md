...
The pre-v1 s-user and s-authn routes were retrofitted in [#73](https://github.com/adam-ai-rob/s-platform/issues/73). The v1 routes are now canonical:

- `GET /user/user/users/me`, `PATCH /user/user/users/me`
- `GET /user/admin/users/{id}`, `GET /user/admin/users`
- `POST /authn/user/sessions:revoke`, `PATCH /authn/user/users/me/password`

The old paths (`/user/me`, `/user/{id}`, `/user/search`, `/authn/user/me/logout`, `/authn/user/me/password`) remain temporarily during the migration window and must return `Deprecation:` + `Sunset:` headers. Do not add new functionality to those legacy paths.
