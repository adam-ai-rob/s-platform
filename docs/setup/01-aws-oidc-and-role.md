...
2. **IAM role `GitHubActionsRole`** with a trust policy scoped to the single `adam-ai-rob/s-platform` repo (narrow — do **not** widen to `adam-ai-rob/*` so forks can never chain-assume the role)
  ...
  "Statement": [
    {
      ...
      "Condition": {
        ...
        "StringLike": {
          "token.actions.githubusercontent.com:sub": "repo:${GH_ORG_OR_USER}/${REPO_NAME}:*"
        }
      }
    }
  ]
