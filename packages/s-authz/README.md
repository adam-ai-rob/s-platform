...
| `POST /authz/admin/users/{userId}/roles/{roleId}` | Assigns a role to a user. Optional body `{ "value": [...] }` scopes assignments for scope-requiring permissions; reassigning unions new scope values with existing ones and returns `204`. |
