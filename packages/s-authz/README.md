...
| `POST /authz/admin/users/{userId}/roles/{roleId}` | Assigns a role to a user. Optional body `{ "value": [...] }` scopes assignments for scope-requiring permissions; reassigning unions new scope values with existing ones and returns `204`. The stored unique scope is capped at 500 entries and 65,536 serialized bytes; invalid or oversized scope values return `400`. |
...
- `400` — malformed request body or assignment scope exceeds limits.
